    ...
    public partial class winClient : Window
    {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                _listPlcInfo = new Model.PlcIpPortInfo().LocalGetAllPlcIP();
            ...
            try
            {
                ...
                if (_IsPlc0Complete)
                {
                    ...
                    if (data != "")
                    {
                        ...
                        // SATOPrinterAPI.s
                        //string productCode = data.Replace("\0", "").Replace("!", "").TrimEnd();
                        Result = response.Split('~')[0];
                        Barcode = response.Split('~')[1];
                        ...
                        try
                        {
                            if (Result == "Y")
                            {
                                ...
                                // string status = printer.GetprinterStatus();

                                // printer.printProductCode(PartNo, Barcode);
                                GlobalVar.Logger.LogMessage(EventNotice.EventTypes.evtInfo, $"GetPLCInput0:{plc.IPAdress}:{plc.PortNo}", $"Received data::{data}");
                                response = $"This Product Data {Barcode} Save Successfully!!!";

                            }
                            else
                            {
                                response = $"NG Data Received ";
                            }
                        }
                        catch (Exception ex)
                        {
                    ...
                    //else
                    //{
                    //    //GlobalVar.Logger.LogMessage(EventNotice.EventTypes.evtData, $"GetPLCInput0:{plc.IPAdress}:{plc.PortNo}", $"No Data found");
                    //}
        ...
        private void NumberInputValidate(TextCompositionEventArgs e)
        {
            Regex regex = new Regex("[^0-9]+");// new Regex("[^0-9]+");
        ...
        private void TxtSerialNo_PreviewTextInput(object sender, TextCompositionEventArgs e)
        {
