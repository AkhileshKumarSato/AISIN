...
class clsPLC_Update : IDisposable
{
    string _IP = "";
    int _Port = 0;
    ...
    internal string GetPLCStatus()
    {
        try
        {
            if (client.Connected)
            {
               // GlobalVar.Logger.LogMessage(EventNotice.EventTypes.evtMessage, $"clsPCL-GetPLCStatus():{_IP}:{_Port}", $"Connected");
                return "Connected";
            }
            else
            {
                GlobalVar.Logger.LogMessage(EventNotice.EventTypes.evtMessage, $"clsPCL-GetPLCStatus():{_IP}:{_Port}", $"Not Connected");
