...
namespace COMServer.Classes
{
    class clsSecurity
    {
        ...
        //New
        internal string GetLineNo()
        {
            clsDB oDb = new clsDB();
            try
            {
                _SbQry = new StringBuilder("Exec [Prc_SaveChildPart] '" + EnumDbType.GET_LINE + "'");
                oDb.Connect();
                DataTable dt = oDb.GetDataTable(_SbQry.ToString());
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        //Column separator $
                        oRule.sResponse += "~" + row["Line_No"].ToString();
                    }
                }
                else
                {
                    oRule.sResponse = clsMsgRule.sInValid + "~There is no pending Line";
                }
            }
            catch (Exception ex) { throw ex; }
            finally
        ...
        internal string ChildMappedSaveData(string DbType,string Binbarcode, string UserID, string LineNo)
        {
            clsDB oDb = new clsDB();
            try
            {
                _SbQry = new StringBuilder("Exec [Prc_SaveChildPart] '" + DbType + "','" + Binbarcode + "','" + UserID + "'");
                _SbQry.AppendLine(",'" + LineNo + "'");
                oDb.Connect();
                DataTable dt = oDb.GetDataTable(_SbQry.ToString());
                if (dt.Rows.Count > 0)
                {
        ...
        internal string GetGET_MAPPED_DATA(string DbType, string Binbarcode, string UserID, string LineNo)
        {
            clsDB oDb = new clsDB();
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        //Column separator $
                        oRule.sResponse += row["KanBanBarcode"].ToString() + "#";
                    }
                }
                else
                {
