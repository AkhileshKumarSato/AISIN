...
namespace AISIN_App
{
    public partial class frmProductionNew : Form
    {
        #region Variables

        //Hold Reference for auto generated Production Id,PRC_PART_BARCODE_FROM_PLC,Prc_Report,Prc_SaveProductionData_New,Prc_SaveChildPart
        ...
        private void btnNext_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dt = oDal.SaveProductionDataNew(oProd);
        ...
        private void txtKanbanBarcode_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                ...
                if (e.KeyChar == 13)
                {
                    ...
                    DataTable dtBin = oDal.GETBinUsingKanban(BarcodeData);
                    ClsGlobal.BinBarcode = "";

                    // txtKanbanBarcode.Text = BarcodeData;
                    oProd.IsBinBarcode = BarcodeData.Length == 22 ? true : false;
                    //If bin barcode then fetch data details
                    if (oProd.IsBinBarcode)
                    {
                        GetBinBarcodeData(BarcodeData);
                    }
                    else if (dtBin.Rows.Count>0)
                    {
                        GetBinBarcodeDataForKanban(dtBin.Rows[0][0].ToString());
                    }
        ...
        private void txtPartBarcode_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                ...
                if (e.KeyChar == 13)
                {
                    ...
                    DataTable dtPartBarcode = oDal.ValidatePartBarcodeFromPLC(txtPartBarcode.Text.Trim());
                    if (ClsGlobal.PCL_Used == "Yes")
                    {
                        if (dtPartBarcode.Rows.Count > 0 && dtPartBarcode.Rows[0][0].ToString() == "Y")
                        {
                            ...
                            string PartBackNo = "";
                            if (PartBarcode.Length == 24)
                            {
                                PartBackNo = PartBarcode.Substring(11, 2);
                            }
                            else
                            {
                                PartBackNo = PartBarcode.Substring(5, 11);
                            }

                            string PartNoFromBackNo = "";

                            DataTable dtPartNo = oDal.GetPartNoFormBackNo(lblBackNo.Text.Trim());


                            if (dtPartNo.Rows.Count > 0)
                            {
                                PartNoFromBackNo = dtPartNo.Rows[0][0].ToString();
                            }
                            else
                            {
                                ClsGlobal.SetInfoMessage("Wrong Part no. " + PartBackNo + " in part barcode " + PartBarcode, lblMessage);
                                txtPartBarcode.Text = "";
                                txtPartBarcode.Focus();
                                return;
                            }
                            ...
                            DataTable dt;
                            if (PartNoFromBackNo == PartBackNo)
                            {
                                oProd.BackNo = lblBackNo.Text.Trim();
                                oProd.CreatedBy = ClsGlobal.UserId;
                                if (ClsGlobal.BinBarcode != "" && ClsGlobal.BinBarcode.Length == 22)
                                {
                                    oProd.KanBanBarcode = ClsGlobal.BinBarcode;//txtKanbanBarcode.Text.Trim();
                                }
                                else
                                {
                                    oProd.KanBanBarcode = txtKanbanBarcode.Text.Trim();
                                ...
                                if (_ProductionId == "")
                                {
                                    ...
                                    if (dt.Rows[0]["Result"].ToString() == "Y")
                                    {
                                        ...
                                        GetPendingKanBanDataNew(); //added by dipak 30-12-20
                                    }
                        ...
                        {
                            ClsGlobal.SetInfoMessage(dtPartBarcode.Rows[0][0].ToString() + " " + PartBarcode, lblMessage);
                            txtPartBarcode.Text = "";
                            txtPartBarcode.Focus();
                        }
                    }
         ...
         */
        private void GetPendingKanBanDataNew()
        {
            try
            {
                ClearAll();
                ...
                DataTable dtBin = oDal.GETBinUsingKanban(oProd.KanBanBarcode);


                if (dtBin.Rows.Count > 0)
                {
                    oProd.KanBanBarcode= dtBin.Rows[0][0].ToString();
                }

                if (oProd.KanBanBarcode != null)
                {
                    oProd.IsBinBarcode = oProd.KanBanBarcode.Length == 22 ? true : false; // Added by Akhilesh on 31-12-2020
                } //End
                ...
                DataSet ds = oDal.GetPendingKanBanDataNew(oProd);
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    else
                    {
                        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                        {
                            ...
                            txtKanbanBarcode.Text = ds.Tables[0].Rows[i]["KanBanBarcode"].ToString();
                        }
                    }
        ...
        private void GetBinBarcodeDataForKanban(string BarcodeData)
        {
            try
            {
                ...
                else //BinBarcode belong to this line
                {
                    ...
                    ClsGlobal.BinBarcode = BarcodeData;
                    DataTable dt = oDal.GetPendingKanBanDataNew(oProd).Tables[0];
        ...
        private void btnDeleteBin_Click(object sender, EventArgs e)
        {
            ClsGlobal.ClearMessage(lblMessage);
            if (txtKanbanBarcode.Text == "")
            {
                ClsGlobal.SetInfoMessage("Invalid Kanban Barcode scanned", lblMessage);
                txtKanbanBarcode.Text = string.Empty;
                txtKanbanBarcode.Clear();
                txtKanbanBarcode.Focus();
                return;
            }
            DialogResult dialogResult = MessageBox.Show("Delete", "Do you want to Delete ?" + txtPartBarcode.Text.Trim().ToString(), MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (dialogResult == DialogResult.No)
            {
                return;
            }

            DataTable dtBin = oDal.GETBinUsingKanban(txtKanbanBarcode.Text.Trim());
            if (dtBin.Rows.Count > 0)
            {
                DataTable dt = oDal.Delete_Existing_Part_Using_Kanban(txtKanbanBarcode.Text.Trim(), dtBin.Rows[0][0].ToString(), ClsGlobal.UserId);
                if (dt.Rows.Count > 0)
                {
                    if (dt.Rows[0]["Result"].ToString() == "Y")
                    {
