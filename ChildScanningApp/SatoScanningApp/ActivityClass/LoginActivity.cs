    ...
    public class LoginActivity : AppCompatActivity
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            try
            {
                ...
                btnUpdateApp.Visibility = ViewStates.Invisible;
        ...
        private void BtnLogin_Click(object sender, EventArgs e)
        {
            try
            {
                if (btnUpdateApp.Visibility == ViewStates.Visible)
                {
        ...
        async Task<string> GetLineNoAsync()
        {
            var progressDialog = ProgressDialog.Show(this, "", "Please wait...", true);
            try
            {
                string[] _RESPONSE = await Task.Run(() => GetLineNoFromServer());
        ...
        private string[] GetLineNoFromServer()
        {
            try
            {
                string _MESSAGE = "GET_LINE_NO~" + "}";
            ...
            try
            {
                ...
                switch (_RESPONSE[0])
                {
                    case "VALID":
                        ...
                        clsGlobal.Line = spinnerPickListNo.SelectedItem.ToString();
        ...
        private string[] SendDataToServer()
        {
            try
            {
                string _MESSAGE = "V_USER~" + editUserId.Text.Trim() + "~" + editPassword.Text.Trim() + "~" + spinnerPickListNo.SelectedItem.ToString() + "}";
                string[] _RESPONSE = oNetwork.fnSendReceiveData(_MESSAGE).Split('~');
